...
画像生成は高速化してきました。短時間で生成できる画像数が格段に増えるとそれに伴う破綻した画像も増加し、また量的処理が追い付かなくなりました。しかし如何にnegative promptやembeddingを駆使しても破綻画像が減りません(というより破綻画像が一切出ない生成システムはいい画像も生成しません)。しかも画像はどれもタグ付けされておらず、ほとんどのクリエイターは手作業での処理となっています。そこで自分が作りたい画像と稀に生成されるクオリティが低い画像をあらかじめ分類するモデルを設計しました。さらに利用者が個人的に欲しい画像とそうでない画像を、ある特徴をもとに分類するようにもなります。
